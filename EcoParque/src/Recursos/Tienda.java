...
public class Tienda {
    ...
    public EcoParque parque;

    public Tienda(EcoParque parque) {
        this.parque = parque;
    }

    public void comprar(Visitante visitante) throws InterruptedException {
        if (!parque.sePuedenRealizarActividades()) {
            System.out.println(ANSI_GREEN + "TIENDA --- " + Thread.currentThread().getName() + " se va de la tienda porque cerró." + ANSI_RESET);
            return;
        }
        Thread.sleep(1000);
        cajas.acquire();
        ...
        while (!esperarFactura.tryAcquire()) {
            if (!parque.sePuedenRealizarActividades()) {
                System.out.println(ANSI_GREEN + "TIENDA --- " + Thread.currentThread().getName() + " se va de la tienda porque cerró." + ANSI_RESET);
                cajas.release();
                return;
